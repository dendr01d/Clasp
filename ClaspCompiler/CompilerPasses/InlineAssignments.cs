    ...
    internal static class InlineAssignments
    {
        public static Prog_Cps Execute(Prog_Cps program)
        {
            Dictionary<Label, ITail> newLabeledTails = program.LabeledTails
                .ToDictionary(x => x.Key, x => InlineInTail(x.Value, new Dictionary<Var, ICpsExp>()));

            return new Prog_Cps(program.Info, newLabeledTails);
        }

        private static ITail InlineInTail(ITail tail, Dictionary<Var, ICpsExp> remap)
        {
            return tail switch
            {
        ...
        private static Return ProcessReturn(Return ret, Dictionary<Var, ICpsExp> remap)
        {
            return new Return(ProcessExpression(ret.Value, remap));
        }

        private static ICpsExp ProcessExpression(ICpsExp exp, Dictionary<Var, ICpsExp> remap)
        {
            return exp switch
            {
                Application app => new Application(app.Operator, app.Arguments.Select(x => ProcessExpression(x, remap))),
                Var v => remap.TryGetValue(v, out ICpsExp? mapOut) ? mapOut : v,
                _ => exp
            };
        }

        private static ITail InlineInSequence(Sequence seq, Dictionary<Var, ICpsExp> remap)
        {
        ...
        private static ITail InlineInConditional(Conditional branch, Dictionary<Var, ICpsExp> remap)
        {
        ...
        private static ITail InlineAssignment(Assignment assgn, ITail tail, Dictionary<Var, ICpsExp> remap)
        {
        ...
        private static bool TryInlineAssignment(Var var, ICpsExp boundValue, ITail tail, Dictionary<Var, ICpsExp> remap,
